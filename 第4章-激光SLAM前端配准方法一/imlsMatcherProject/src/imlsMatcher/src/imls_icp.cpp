                                         ...
                                         double& height)
{
    ...
    {
        double dis = (x - nearPoints[i]).norm() * (x - nearPoints[i]).norm(); 
        double Wxi = std::exp(-dis / (m_h * m_h));
        Wx.push_back(Wxi);
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    for(int i = 0; i < nearPoints.size(); i++)
    {
        centerX += nearPoints[i](0);
        centerY += nearPoints[i](1);
    }

    centerX /= nearPoints.size();
    centerY /= nearPoints.size();
    ...
    for(int i = 0; i < nearPoints.size(); i++)
    {
        sigma += (nearPoints[i] - center) * (nearPoints[i] - center).transpose();
    }
    ...
    eigenValue.rowwise().sum().minCoeff(&minIndex);

    normal = eigenVector.col(minIndex);
    //end of TODO
