...
//此处bag包的地址需要自行修改
std::string bagfile = "/home/zy/Downloads/HW4/imlsMatcherProject/src/bag/imls_icp.bag";
...
public:
    ...
    void championLaserScanCallback(const champion_nav_msgs::ChampionNavLaserScanConstPtr& msg)
    {
        ...
        if(m_prevLDP != NULL)
        {
            ...
            d_point_scan = PIICPBetweenTwoFrames(currentLDP, Eigen::Vector3d(0,0,0));
        }
        else
        {
            LaserScanToLDP(msg, m_prevLDP);
        ...
        lastPose << cos(m_prevLaserPose(2)), -sin(m_prevLaserPose(2)), 0, sin(m_prevLaserPose(2)), cos(m_prevLaserPose(2)), 0, 0, 0, 1;

        m_prevLaserPose += lastPose * dPose;
    ...
    void LaserScanToLDP(const champion_nav_msgs::ChampionNavLaserScanConstPtr& pScan, LDP& ldp)
    {
       ...
       for(int i = 0; i < nPts; i++)
       {
           ...
           double angle = pScan->angles[i];
           if(dist > pScan->range_min && dist < pScan->range_max && angle > pScan->angle_min && angle < pScan->angle_max)
           {
               ldp->valid[i] = 1;
               ldp->readings[i] = dist;
           }
