...
void ReadPoses(const std::string path, std::map<int, Pose2d>& poses)
{
    ...
    while(std::getline(fin, line))
    {
        ...
        pose_2d.x = x;
        pose_2d.y = y;
        pose_2d.yaw_radians = theta;

        poses.insert(std::make_pair(id, pose_2d));
    }
...
void ReadConstraints(const std::string path, std::vector<Constraint2d>& constraints)
{
    ...
    while (std::getline(fin, line))
    {
        ...
        Constraint2d constraint_2d;
        constraint_2d.id_begin = xi;
        constraint_2d.id_end = xj;
        constraint_2d.x = dx;
        constraint_2d.y = dy;
        constraint_2d.yaw_radians = dtheta;
        constraint_2d.information << inf_xx, inf_xy, inf_xt,
                                     inf_xy, inf_yy, inf_yt,
                                     inf_xt, inf_yt, inf_tt;

        constraints.push_back(constraint_2d);
    }
...
bool OutputPoses(const std::string filename, const std::map<int, Pose2d>& poses)
{
    ...
    for(std::map<int, Pose2d>::const_iterator poses_iter = poses.begin(); poses_iter != poses.end(); ++poses_iter)
    {
