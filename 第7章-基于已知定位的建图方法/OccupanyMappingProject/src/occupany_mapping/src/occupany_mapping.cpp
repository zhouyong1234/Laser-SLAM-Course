...
void OccupanyMapping(std::vector<GeneralLaserScan> &scans, std::vector<Eigen::Vector3d> &robot_poses)
{
    ...
    for (int i = 0; i < scans.size(); i++)
    {
        ...
        for (int id = 0; id < scan.range_readings.size(); id++)
        {
            ...
            if(!isValidGridIndex(mapIndex))
                continue;

            std::vector<GridIndex> free_index = TraceLine(robotIndex.x, robotIndex.y, mapIndex.x, mapIndex.y);

            #if 0
            for(int k = 0; k < free_index.size(); k++)
            {
                int line_index = GridIndexToLinearIndex(free_index[k]);
                int value = pMap[line_index];
                value += mapParams.log_free;

                if(value < 0)
                    pMap[line_index] = 0;
                else
                    pMap[line_index] = value;

            }

            int occ_index = GridIndexToLinearIndex(mapIndex);
            pMap[occ_index] += mapParams.log_occ;
            if(pMap[occ_index] > 100)
                pMap[occ_index] = 100;
            #else
            for(int j = 0; j < free_index.size(); j++)
            {
                int line_index = GridIndexToLinearIndex(free_index[j]);
                ++pMapMisses[line_index];
            }
            int occ_index = GridIndexToLinearIndex(mapIndex);
            ++pMapHits[occ_index];
    ...
    for(int i = 0; i < mapParams.height * mapParams.width; i++)
    {
        ...
        {
            int m = 100 * pMapHits[i] / (pMapHits[i] + pMapMisses[i]);
            if(m > 35)
                pMap[i] = 100;
            else
                pMap[i] = m;
    ...
    #if 0
    for(int i = 0; i < mapParams.height-2;i++)
    {
        for(int j = 0; j < mapParams.width-2;j++)
        {
            int line_value = j + i * mapParams.height;
            int line_x = line_value + 1;
            int line_y = line_value + mapParams.height;
            ...
            double a,b,b1,x,y;
            a = pMapTSDF[line_value];
            b = pMapTSDF[line_x];
            b1 = pMapTSDF[line_y];
            if(a*b1 < 0)
            {
                x = A_x;
                y = interpolation(A_y, B_y, a, b1);
...
int main(int argc, char **argv)
{
    ...
    std::string basePath = "/home/touchair/Downloads/HW7/OccupanyMappingProject/src/data";
